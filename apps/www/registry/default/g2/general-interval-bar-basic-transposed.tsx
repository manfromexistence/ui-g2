...
export default function G2ChartComponent_general_interval_bar_basic_transposed() {
  ...
  useEffect(() => {
    ...
    if (chartRef.current && !g2ChartInstance.current) {
      try {
        ...
        console.error(
          "Error initializing G2 chart from integration/G2/site/examples/general/interval/demo/bar-basic-transposed.ts:",
          error
        )
        if (chartRef.current) {
          chartRef.current.innerHTML =
            '<div style="color: red; text-align: center; padding: 20px;">Failed to render G2 chart. Check console for errors. Source: integration/G2/site/examples/general/interval/demo/bar-basic-transposed.ts</div>'
        }
      }
    }
    ...
    return () => {
      if (g2ChartInstance.current) {
        try {
          ...
          console.error(
            "Error destroying G2 chart from integration/G2/site/examples/general/interval/demo/bar-basic-transposed.ts:",
            e
          )
        }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Simple Bar</CardTitle>
        <CardDescription>
          G2 Chart. Original example:
          general/interval/demo/bar-basic-transposed.ts
        </CardDescription>
      </CardHeader>
      <CardContent>
