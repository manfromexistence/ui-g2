  ...
  useEffect(() => {
    ...
    if (chartRef.current && !g2ChartInstance.current) {
      try {
        ...
        console.error(
          "Error initializing G2 chart from integration/G2/site/examples/accessible/text-searching/demo/text-search.ts:",
          error
        )
        if (chartRef.current) {
          chartRef.current.innerHTML =
            '<div style="color: red; text-align: center; padding: 20px;">Failed to render G2 chart. Check console for errors. Source: integration/G2/site/examples/accessible/text-searching/demo/text-search.ts</div>'
        }
      }
    }
    ...
    return () => {
      if (g2ChartInstance.current) {
        try {
          ...
          console.error(
            "Error destroying G2 chart from integration/G2/site/examples/accessible/text-searching/demo/text-search.ts:",
            e
          )
        }
    ...
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          ...
          accessible/text-searching/demo/text-search.ts
        </CardDescription>
      </CardHeader>
      <CardContent>
