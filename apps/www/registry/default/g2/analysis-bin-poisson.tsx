...
export default function G2ChartComponent_analysis_bin_poisson() {
  ...
  useEffect(() => {
    ...
    if (chartRef.current && !g2ChartInstance.current) {
      try {
        ...
        console.error(
          "Error initializing G2 chart from integration/G2/site/examples/analysis/bin/demo/poisson.ts:",
          error
        )
        if (chartRef.current) {
          chartRef.current.innerHTML =
            '<div style="color: red; text-align: center; padding: 20px;">Failed to render G2 chart. Check console for errors. Source: integration/G2/site/examples/analysis/bin/demo/poisson.ts</div>'
        }
      }
    }
    ...
    return () => {
      if (g2ChartInstance.current) {
        try {
          ...
          console.error(
            "Error destroying G2 chart from integration/G2/site/examples/analysis/bin/demo/poisson.ts:",
            e
          )
        }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Poisson distribution</CardTitle>
        <CardDescription>
          G2 Chart. Original example: analysis/bin/demo/poisson.ts
        </CardDescription>
      </CardHeader>
      <CardContent>
