...
export default function G2ChartComponent_general_helix_helix() {
  ...
  useEffect(() => {
    ...
    if (chartRef.current && !g2ChartInstance.current) {
      try {
        ...
        console.error(
          "Error initializing G2 chart from integration/G2/site/examples/general/helix/demo/helix.ts:",
          error
        )
        if (chartRef.current) {
          chartRef.current.innerHTML =
            '<div style="color: red; text-align: center; padding: 20px;">Failed to render G2 chart. Check console for errors. Source: integration/G2/site/examples/general/helix/demo/helix.ts</div>'
        }
      }
    }
    ...
    return () => {
      if (g2ChartInstance.current) {
        try {
          ...
          console.error(
            "Error destroying G2 chart from integration/G2/site/examples/general/helix/demo/helix.ts:",
            e
          )
        }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Helix</CardTitle>
        <CardDescription>
          G2 Chart. Original example: general/helix/demo/helix.ts
        </CardDescription>
      </CardHeader>
      <CardContent>
