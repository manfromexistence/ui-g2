...
export default function G2ChartComponent_general_dual_multi_line_sync() {
  ...
  function syncTicksOfDomainsFromZero(scales) {
    ...
    const normalize = (d) =>
      d / Math.pow(10, Math.ceil(Math.log(d) / Math.LN10))
  ...
  useEffect(() => {
    ...
    if (chartRef.current && !g2ChartInstance.current) {
      try {
        ...
        console.error(
          "Error initializing G2 chart from integration/G2/site/examples/general/dual/demo/multi-line-sync.ts:",
          error
        )
        if (chartRef.current) {
          chartRef.current.innerHTML =
            '<div style="color: red; text-align: center; padding: 20px;">Failed to render G2 chart. Check console for errors. Source: integration/G2/site/examples/general/dual/demo/multi-line-sync.ts</div>'
        }
      }
    }
    ...
    return () => {
      if (g2ChartInstance.current) {
        try {
          ...
          console.error(
            "Error destroying G2 chart from integration/G2/site/examples/general/dual/demo/multi-line-sync.ts:",
            e
          )
        }
