...
// Original G2 example path: integration/G2/site/examples/interaction/data/demo/area-normalizeY-element-point-move.ts

const FALLBACK_COLORS_JSON =
  '["#E57373","#81C784","#64B5F6","#FFD54F","#BA68C8"]'

export default function G2ChartComponent_interaction_data_area_normalizey_element_point_move() {
  ...
  useEffect(() => {
    ...
    if (chartRef.current && !g2ChartInstance.current) {
      try {
        ...
        console.error(
          "Error initializing G2 chart from integration/G2/site/examples/interaction/data/demo/area-normalizeY-element-point-move.ts:",
          error
        )
        if (chartRef.current) {
          chartRef.current.innerHTML =
            '<div style="color: red; text-align: center; padding: 20px;">Failed to render G2 chart. Check console for errors. Source: integration/G2/site/examples/interaction/data/demo/area-normalizeY-element-point-move.ts</div>'
        }
      }
    }
    ...
    return () => {
      if (g2ChartInstance.current) {
        try {
          ...
          console.error(
            "Error destroying G2 chart from integration/G2/site/examples/interaction/data/demo/area-normalizeY-element-point-move.ts:",
            e
          )
        }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Area NormalizeY Numerical Interaction</CardTitle>
        <CardDescription>
          G2 Chart. Original example:
          interaction/data/demo/area-normalizeY-element-point-move.ts
        </CardDescription>
      </CardHeader>
      <CardContent>
