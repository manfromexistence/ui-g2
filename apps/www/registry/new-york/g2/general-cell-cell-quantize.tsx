...
// Original G2 example path: integration/G2/site/examples/general/cell/demo/cell-quantize.ts

const FALLBACK_COLORS_JSON =
  '["#E57373","#81C784","#64B5F6","#FFD54F","#BA68C8"]'

export default function G2ChartComponent_general_cell_cell_quantize() {
  ...
  useEffect(() => {
    ...
    if (chartRef.current && !g2ChartInstance.current) {
      try {
        ...
        console.error(
          "Error initializing G2 chart from integration/G2/site/examples/general/cell/demo/cell-quantize.ts:",
          error
        )
        if (chartRef.current) {
          chartRef.current.innerHTML =
            '<div style="color: red; text-align: center; padding: 20px;">Failed to render G2 chart. Check console for errors. Source: integration/G2/site/examples/general/cell/demo/cell-quantize.ts</div>'
        }
      }
    }
    ...
    return () => {
      if (g2ChartInstance.current) {
        try {
          ...
          console.error(
            "Error destroying G2 chart from integration/G2/site/examples/general/cell/demo/cell-quantize.ts:",
            e
          )
        }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Quantize Heatmap</CardTitle>
        <CardDescription>
          G2 Chart. Original example: general/cell/demo/cell-quantize.ts
        </CardDescription>
      </CardHeader>
      <CardContent>
