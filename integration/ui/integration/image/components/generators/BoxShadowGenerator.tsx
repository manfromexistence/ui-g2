...
export function BoxShadowGenerator() {
  ...
  const [blurRadius, setBlurRadius] = useState<number>(32);
  const [spreadRadius, setSpreadRadius] = useState<number>(0);
  const [color, setColor] = useState<string>("#1f2687"); // Default shadow color
  const [opacity, setOpacity] = useState<number>(0.37);
  const [inset, setInset] = useState<boolean>(false);

  const generateCss = () => {
    const shadowColorRgba = hexToRgba(color, opacity);
    const insetKeyword = inset ? "inset " : "";
    return `box-shadow: ${insetKeyword}${offsetX}px ${offsetY}px ${blurRadius}px ${spreadRadius}px ${shadowColorRgba};`;
  };
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(cssCode).then(() => {
      console.log("Box Shadow CSS copied!");
    }).catch(err => {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col space-y-4 p-4"> {/* Reduced space-y */}
          <h3 className="text-xl font-semibold text-white">Customize</h3>

          {/* Offset X */}
          <div className="space-y-2">
            <Label htmlFor="offset-x" className="block text-sm font-medium text-white">
              Offset X: {offsetX}px
            </Label>
            <Slider id="offset-x" min={-50} max={50} step={1} value={[offsetX]} onValueChange={(v) => setOffsetX(v[0])} className="slider-styling" />
          </div>

          {/* Offset Y */}
          <div className="space-y-2">
            <Label htmlFor="offset-y" className="block text-sm font-medium text-white">
              Offset Y: {offsetY}px
            </Label>
            <Slider id="offset-y" min={-50} max={50} step={1} value={[offsetY]} onValueChange={(v) => setOffsetY(v[0])} className="slider-styling" />
          </div>

          {/* Blur Radius */}
          <div className="space-y-2">
            <Label htmlFor="blur-radius" className="block text-sm font-medium text-white">
              Blur Radius: {blurRadius}px
            </Label>
            <Slider id="blur-radius" min={0} max={100} step={1} value={[blurRadius]} onValueChange={(v) => setBlurRadius(v[0])} className="slider-styling" />
          </div>

          {/* Spread Radius */}
          <div className="space-y-2">
            <Label htmlFor="spread-radius" className="block text-sm font-medium text-white">
              Spread Radius: {spreadRadius}px
            </Label>
            <Slider id="spread-radius" min={-50} max={50} step={1} value={[spreadRadius]} onValueChange={(v) => setSpreadRadius(v[0])} className="slider-styling" />
          </div>
          ...
          <div className="flex items-end space-x-4">
            ...
            <div className="space-y-2 w-1/3">
              <Label htmlFor="shadow-opacity" className="block text-sm font-medium text-white">Opacity: {opacity.toFixed(2)}</Label>
              <Slider id="shadow-opacity" min={0} max={1} step={0.01} value={[opacity]} onValueChange={(v) => setOpacity(v[0])} className="slider-styling" />
            </div>
          </div>

          {/* Inset Checkbox */}
          <div className="flex items-center space-x-2 pt-2">
            <Checkbox id="inset-check" checked={inset} onCheckedChange={(checked) => setInset(Boolean(checked))} className="border-white/50 data-[state=checked]:bg-blue-500" />
            <Label htmlFor="inset-check" className="text-sm font-medium text-white leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
              Inset Shadow
            </Label>
          </div>
      ...
      <style jsx>{`
        .slider-styling {
          /* Example: Apply shadcn classes directly or define custom styles */
